package com.updo.excelfile.util;
...
public class ExcelUtils {
	...
	public static ByteArrayInputStream customersToExcel(List<Customer> customers) throws IOException {
		String[] COLUMNs = {"Id", "Name", "Address", "Age","EsiId"};
		try(
			...
			Sheet sheet = workbook.createSheet("Employees-Download");
			...
			for (Customer customer : customers) {
				...
				row.createCell(4).setCellValue(customer.getEsiId());
			}
		...
		try {
    		...
    		while (rows.hasNext()) {
    			...
    			while (cellsInRow.hasNext()) {
    				...
    				else if(cellIndex==4) { // EsiId
    					cust.setEsiId( currentCell.getStringCellValue());
    				}
